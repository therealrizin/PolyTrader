...
@Slf4j
@Service
public class RealBetService<PolymarketMarketResolver> {
    ...
    /**
     * Prevent two strategy ticks from placing two bets on the same
     * 5-minute market.
     */
    ...
    /**
     * Locally tracked real bets.
     *
     * This is intentionally lightweight. Polymarket remains the source
     * of truth for the actual order.
     */
    ...
    public RealBet placeRealBet(
        ...
        try {
                    ...
                    marketResolver.resolveCurrentMarket()
                            ...
                            .orElseThrow(() ->
                                    new IllegalStateException(
                                            "Could not resolve token IDs for "
                                                    + slug
                                    )
                            );
             ...
             * size  = USDC amount / token price
             *
             * Therefore:
             *
             * - we never pay more than the price observed by Java
             * - FOK prevents leaving a resting order behind
             * - if sufficient liquidity is unavailable, the order fails
             */
            BigDecimal size =
                    amount.divide(
                            price,
            ...
            log.info(
                    "REAL BET submitting: " +
                            "id={} slug={} side={} tokenId={} " +
                            "price={} size={} amount={} EV={} winChance={} " +
                            "secondsUntilClose={}",
                    clientBetId,
                    slug,
                    side,
            ...
            /*
             * The order was not successfully accepted by our execution
             * service, so allow another attempt for this market.
             */
    ...
    private void validatePrice(BigDecimal price) {
        ...
        /*
         * Polymarket prediction-market prices should be in
         * approximately [0,1].
         */
    }
