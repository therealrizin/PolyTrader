...
public class ChainlinkPriceStreamClient {
    ...
    public synchronized void stop() {
        ...
        log.info("Chainlink RTDS stream stopped");
    }
    ...
    private void connect() {
        ...
        client.execute(new TextWebSocketHandler() {
            ...
            protected void handleTextMessage(WebSocketSession session, TextMessage message) {
                ...
                // This timestamp only tracks RTDS socket activity; NOT used for price freshness.
                lastMessageAtMillis = System.currentTimeMillis();
    ...
    private void sendSubscription(WebSocketSession session) {
        try {
            ...
            String subscribeMsg = objectMapper.writeValueAsString(
                    Map.of("action", "subscribe", "subscriptions", subscriptions)
            );
            session.sendMessage(new TextMessage(subscribeMsg));
            log.info("Subscribed to Polymarket Chainlink RTDS topics: {}", subscriptions);

        } catch (Exception e) {
            log.error("Failed to subscribe to Polymarket RTDS", e);
        }
    }
    ...
    private void handleRawPrice(ChainlinkSymbol symbol, JsonNode payload) {
        ...
        // Never update raw price without a timestamp.
    ...
    private void handleTwapSixty(ChainlinkSymbol symbol, JsonNode payload) {
        ...
        if (symbol == ChainlinkSymbol.BTC_USD) {
            ...
            if (obsTimestampNode == null || obsTimestampNode.isNull()) {
                log.warn(
    ...
    private BigDecimal parseTwapValue(JsonNode payload) {
        ...
        if (fullAccuracyNode != null && !fullAccuracyNode.isNull()) {
            try {
                ...
                log.warn("Failed to parse Chainlink TWAP full_accuracy_value: {}", fullAccuracyNode, e);
            }
        }
    ...
    private BigDecimal parseDecimal(JsonNode node) {
        ...
        try {
            ...
            log.warn("Failed to parse decimal RTDS value: {}", node, e);
            return null;
        }
    }
    ...
    private void checkStaleness() {
        if (!running || lastMessageAtMillis < 0) {
            return;
        }
        ...
        if (silentFor > STALE_THRESHOLD.toMillis()) {
            ...
            if (session != null && session.isOpen()) {
                try {
                    ...
                    log.debug("Failed to close stale RTDS session", e);
                }
            }
