...
public class PolymarketDataProvider {
    ...
    public PolymarketDataProvider(@Qualifier("gammaWebClient") WebClient gammaWebClient,
                                  @Qualifier("polymarketWebClient") WebClient polymarketWebClient,
    ...
    private void onPriceUpdate(String callbackSlug, MarketSide side, BigDecimal bestBid, BigDecimal bestAsk) {
        ...
        if (slug == null || !slug.equals(callbackSlug)) return;
        switch (side) {
            case UP -> {
                if (bestBid != null) upBestBid.set(bestBid);
                if (bestAsk != null) upBestAsk.set(bestAsk);
            }
            case DOWN -> {
                if (bestBid != null) downBestBid.set(bestBid);
                if (bestAsk != null) downBestAsk.set(bestAsk);
            }
        }
        rebuildSnapshot(callbackSlug);
    }
    ...
    private void refresh() {
        try {
            ...
            if (event == null) {
                ...
                if (slug.equals(currentSlug.get())) latestSnapshot.set(null);
                return;
            }
            ...
            if (strikePriceUsd == null) {
                ...
                if (!slug.equals(currentSlug.get())) return;
                ...
                log.debug("REFRESH result=USING_CACHED_PRICE_TO_BEAT slug={} priceToBeat={}", slug, strikePriceUsd);
            }
            if (!slug.equals(currentSlug.get())) return;
            currentEndEpochMillis.set(endEpochMillis);
            currentWindowOpenEpochMillis.set(windowOpenEpochMillis);
            marketWebSocketClient.subscribe(slug, (side, bid, ask) -> onPriceUpdate(slug, side, bid, ask));
    ...
    private void rebuildSnapshot(String slug) {
        Long endMillis = currentEndEpochMillis.get();
        Long openMillis = currentWindowOpenEpochMillis.get();
        if (endMillis == null || openMillis == null) return;
        long nowMillis = polymarketClock.nowMillis();
        rebuildSnapshot(slug, secondsUntil(endMillis, nowMillis), secondsSince(openMillis, nowMillis));
    }

    private void rebuildSnapshot(String slug, long secondsUntilClose, long secondsSinceOpen) {
        if (!slug.equals(currentSlug.get())) return;
        BigDecimal strikePriceUsd = referenceOpenPrice.get();
        ...
        log.debug("SNAPSHOT UPDATED slug={} UP[bid={},ask={}] DOWN[bid={},ask={}] secondsLeft={} priceToBeat={}",
                slug, upBid, upAsk, downBid, downAsk, secondsUntilClose, strikePriceUsd);
    }

    private void handleWindowChange(String slug, long windowOpenEpochMillis, long endEpochMillis) {
        String previousSlug = currentSlug.getAndSet(slug);
        if (slug.equals(previousSlug)) return;
        ...
        lastSnapshotHeartbeatAt.set(Instant.EPOCH);
        log.info("REFRESH result=NEW_WINDOW slug={} previousSlug={}", slug, previousSlug);
    }

    private BigDecimal fetchPriceToBeat(long eventStartEpochMillis, long endEpochMillis) {
        String eventStartTime = Instant.ofEpochMilli(eventStartEpochMillis).toString();
        String endDate = Instant.ofEpochMilli(endEpochMillis).toString();
        try {
            ResponseEntity<JsonNode> response = polymarketWebClient.get()
                    .uri(uriBuilder -> uriBuilder.path(CRYPTO_PRICE_PATH)
                            .queryParam("symbol", BTC_CRYPTO_SYMBOL)
                            .queryParam("eventStartTime", eventStartTime)
                            .queryParam("variant", CRYPTO_VARIANT)
                            .queryParam("endDate", endDate)
                            .queryParam("twapEnabled", TWAP_ENABLED)
                            .queryParam("twapLookbackSeconds", TWAP_LOOKBACK_SECONDS)
                            .build())
                    ...
                    .onErrorResume(e -> {
                        log.warn("PRICE_TO_BEAT HTTP_FAILED eventStart={} endDate={} error={}",
                                eventStartTime, endDate, e.getMessage());
                        return Mono.empty();
                    })
            ...
            JsonNode openPriceNode = body.get("openPrice");
            if (openPriceNode == null || openPriceNode.isNull()) {
                log.warn("PRICE_TO_BEAT result=NO_OPEN_PRICE eventStart={} endDate={} response={}",
                        eventStartTime, endDate, body);
                return null;
            }
            BigDecimal openPrice = parsePrice(openPriceNode);
            if (openPrice == null || openPrice.signum() <= 0) {
                log.warn("PRICE_TO_BEAT result=INVALID_OPEN_PRICE openPrice={}", openPriceNode);
                return null;
            }
            log.info("PRICE_TO_BEAT result=SUCCESS priceToBeat={} source=POLYMARKET_CRYPTO_PRICE_API " +
                            "eventStartTime={} endDate={} completed={} incomplete={} cached={}",
                    openPrice, eventStartTime, endDate,
                    getBooleanField(body, "completed"),
                    getBooleanField(body, "incomplete"),
                    getBooleanField(body, "cached"));
            return openPrice;
        } catch (Exception e) {
            log.error("PRICE_TO_BEAT result=ERROR eventStart={} endDate={} error={}",
                    eventStartTime, endDate, e.getMessage(), e);
            return null;
        }
    }
    ...
    private Long parseEndDate(JsonNode market) {
        JsonNode endDateNode = market.get("endDate");
        if (endDateNode == null || endDateNode.isNull() || endDateNode.asText().isBlank()) return null;
        try {
            return Instant.parse(endDateNode.asText()).toEpochMilli();
        } catch (Exception e) {
            log.warn("Could not parse Polymarket endDate={}", endDateNode);
            return null;
        }
    }

    private String computeCurrentSlug() {
        long nowSeconds = polymarketClock.nowMillis() / 1000;
        long windowSeconds = WINDOW.toSeconds();
        long windowStart = (nowSeconds / windowSeconds) * windowSeconds;
        return BTC_SLUG_PREFIX + windowStart;
    }

    private long secondsUntil(long endEpochMillis, long nowMillis) {
        return Math.max(0, (endEpochMillis - nowMillis) / 1000);
    }

    private long secondsSince(long startEpochMillis, long nowMillis) {
        return Math.max(0, (nowMillis - startEpochMillis) / 1000);
    }

    private JsonNode fetchEventBySlug(String slug) {
        ResponseEntity<JsonNode> response = gammaWebClient.get()
                .uri(uriBuilder -> uriBuilder.path("/events").queryParam("slug", slug).build())
                .retrieve()
                .toEntity(JsonNode.class)
                .onErrorResume(e -> {
                    log.warn("Gamma event lookup failed slug={} error={}", slug, e.getMessage());
                    return Mono.empty();
                })
                .block();
        if (response == null) return null;
        ...
        if (body.isArray()) return body.isEmpty() ? null : body.get(0);
        return body;
    }

    private void recordGammaClockSample(String httpDateHeader) {
        if (httpDateHeader == null || httpDateHeader.isBlank()) return;
        try {
            ...
            log.debug("Failed to parse Gamma Date header '{}'", httpDateHeader);
        }
    }
}