...
public class TradingEngine {
    ...
    private double estimatedUpChance(
        ...
        if (secondsLeft >= TWAP_WINDOW_SECONDS) {
            ...
            return probabilityOfReaching(requiredPctChange, secondsLeft);
        }
    ...
    private double probabilityOfReaching(
        ...
        if (requiredPctChange < 0.0) {
            double probabilityDown = table.getChance(secondsLeft, Math.abs(requiredPctChange));
            return clampProbability(1.0 - probabilityDown);
        }

        double probabilityUp = table.getChance(secondsLeft, requiredPctChange);
    ...
    public double netSellValuePerShare(double currentBid, double takerFee) {
        if (currentBid <= 0.0 || currentBid >= 1.0) {
            return 0.0;
        }
        double fee = takerFee * currentBid * (1.0 - currentBid);
